...
function setupAlarm() {
  ...
  console.log(
    `[Video Downloader] Alarm set for every ${SYNC_INTERVAL_MINUTES} minutes`
  );
}
...
chrome.runtime.onMessageExternal.addListener(
  (message, sender, sendResponse) => {
    console.log(
      "[Video Downloader] External message received:",
      message.type,
      "from:",
      sender.origin
    );
    ...
    const isAllowed = allowedOrigins.some(
      (origin) => sender.origin === origin || sender.origin.startsWith(origin)
    );

    if (!isAllowed) {
      console.warn(
        "[Video Downloader] Rejected message from untrusted origin:",
        sender.origin
      );
    ...
    if (message.type === "PAIRING_CODE") {
      ...
      chrome.storage.local
        ...
        .then(() => {
          console.log(
            "[Video Downloader] Pairing code saved to storage:",
            message.code
          );
        });

      // Background'da direkt pairing yap (popup'a bağımlı olmadan)
      handleBackgroundPairing(message.code, message.serverUrl).then(
        (success) => {
          if (success) {
...
async function handleBackgroundPairing(code, serverUrl) {
  ...
  serverUrl = (
    serverUrl || "https://video-downloader-production-5a88.up.railway.app"
  ).replace(/\/$/, "");
  ...
  try {
    ...
    console.log(
      "[Video Downloader] Attempting background pairing with code:",
      code
    );
...
async function syncCookiesBackground(serverUrl, token) {
  try {
    ...
    console.log(
      `[Video Downloader] Syncing ${allCookies.length} cookies after pairing`
    );
