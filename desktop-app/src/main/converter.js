...
function parseFFmpegOutput(output, filePath) {
    ...
    const videoMatch = output.match(/Stream\s+#\d+:\d+.*Video:\s+(\w+).*?,\s*(\d+)x(\d+)/);
    if (videoMatch) {
        info.video = {
            codec: videoMatch[1],
    ...
    const audioMatch = output.match(/Stream\s+#\d+:\d+.*Audio:\s+(\w+).*?,\s*(\d+)\s*Hz/);
    if (audioMatch) {
        info.audio = {
            codec: audioMatch[1],
...
async function convertFile(options, onProgress) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (formatConfig.type === 'video') {
            // Video conversion
            args.push(
                '-c:v', formatConfig.codec,
                '-crf', presetConfig.video.crf.toString(),
                '-preset', presetConfig.video.preset,
                '-c:a', 'aac',
                '-b:a', presetConfig.audio.bitrate
            );
        ...
        getFileInfo(inputPath).then(info => {
        ...
        activeProcess.on('close', (code) => {
            ...
            if (code === 0) {
                if (onProgress) {
                    onProgress({ percent: 100, currentTime: duration, totalTime: duration });
                }
                resolve({
                    success: true,
                    inputPath,
                ...
                (progress) => {
                    if (onProgress) {
                        onProgress({
                            ...
                            overallProgress: Math.round(((i + progress.percent / 100) / totalFiles) * 100)
                        });
                    }
                }
            );
