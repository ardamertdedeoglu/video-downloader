...
async function checkBinariesStatus() {
  ...
  if (ytdlpExists) {
    try {
      ytdlpVersion = execSync(`"${ytdlpPath}" --version`, {
        encoding: "utf8",
      }).trim();
    } catch (e) {
...
async function updateYtdlp(onProgress) {
  ...
  await downloadFile(BINARY_URLS[PLATFORM]["yt-dlp"], ytdlpPath, (percent) =>
    onProgress({ step: "yt-dlp", percent, status: "downloading" })
  );
  ...
  // Return new version
  try {
    const newVersion = execSync(`"${ytdlpPath}" --version`, {
      encoding: "utf8",
    }).trim();
...
async function downloadBinaries(onProgress) {
  ...
  const archivePath = path.join(
    BINARIES_DIR,
    isZip ? "ffmpeg.zip" : "ffmpeg.tar.xz"
  );

  await downloadFile(ffmpegUrl, archivePath, (percent) =>
    onProgress({
      step: "ffmpeg",
      percent: percent * 0.8,
      status: "downloading",
  ...
  await downloadFile(denoUrl, denoArchivePath, (percent) =>
    onProgress({ step: "deno", percent: percent * 0.9, status: "downloading" })
  );
...
async function extractFfmpeg(archivePath, destDir) {
  ...
  if (archivePath.endsWith(".zip")) {
    ...
    for (const entry of entries) {
      ...
      if (
        name.endsWith("ffmpeg.exe") ||
        (name.endsWith("ffmpeg") && !name.includes("."))
      ) {
